 ...
 * --------------------------------------------------------------------------
 * Código completo com correção: widget abre corretamente ao clicar no ícone
 * após colapsar/expandir. Também reaplica listeners necessários ao recriar
 * o conteúdo do root.
 * ==========================================================================
*/
...
(() => {
  ...
  function injectCSS(){
    const T = CFG.THEME;
    const css = `
      .wbq-root{position:fixed;z-index:999999;display:flex;align-items:center;justify-content:center;background:${T.panel};color:${T.text};border:1px solid ${T.border};box-shadow:0 12px 30px rgba(0,0,0,.6);border-radius:12px;overflow:hidden;font-family:Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",Roboto,"Helvetica Neue",Arial}
      .wbq-collapsed{width:64px;height:64px;cursor:grab;user-select:none;padding:8px}
      ...
      .wbq-body{padding:10px;height:calc(100% - 92px);overflow:auto;background:${T.bg}}
      ...
      @media (max-width:420px){ .wbq-expanded{width:320px;height:420px} }
    `;
    let st = document.getElementById('wbq-style');
    if (!st) {
  ...
  function buildUI(){
    ...
    // eventos primários: dblclick para abrir, mousedown para arraste
    root.addEventListener('dblclick', toggleExpand);
    root.addEventListener('mousedown', dragStartHandler);
    root.addEventListener('touchstart', dragStartHandler, { passive: false });

    // garantir que clicar no ícone abra (listener no elemento raiz também captura, mas colocamos listener direto para robustez)
    const icon = root.querySelector('.wbq-icon');
    if (icon) {
  ...
  function renderExpanded(){
    ...
    // permitir arrastar também a partir do header (compat)
    const dragHeader = r.querySelector('#wbqDrag');
    if (dragHeader) {
      dragHeader.addEventListener('mousedown', dragStartHandler);
      dragHeader.addEventListener('touchstart', dragStartHandler, { passive: false });
    }
    ...
    // config inputs
    r.querySelector('#wbqArea').addEventListener('change', e => { CFG.AREA = clamp(parseInt(e.target.value,10)||100, 10, 300); savePrefs(); updateStatsUI(); });
    r.querySelector('#wbqStartX').addEventListener('change', e => { CFG.START_X = parseInt(e.target.value,10)||CFG.START_X; savePrefs(); updateStatsUI(); });
    r.querySelector('#wbqStartY').addEventListener('change', e => { CFG.START_Y = parseInt(e.target.value,10)||CFG.START_Y; savePrefs(); updateStatsUI(); });
  ...
  function toggleExpand(){
    ...
    if (S.expanded) {
      ...
      // colapsar: trocar conteudo para o ícone e reanexar listeners mínimos
      r.classList.add('wbq-collapsed');
      r.classList.remove('wbq-expanded');
      r.innerHTML = `<div class="wbq-icon" title="Wplace Bot">🎨</div>`;
      // reanexar listener de arraste (root já tem mousedown, mas asseguramos)
      r.addEventListener('mousedown', dragStartHandler);
      r.addEventListener('touchstart', dragStartHandler, { passive: false });
      // adicionar listener direto ao ícone para garantir que clique abra
      const icon = r.querySelector('.wbq-icon');
      if (icon) {
        icon.addEventListener('click', (ev) => {
          ev.stopPropagation();
          if (!S.expanded) toggleExpand();
  ...
  function getClient(ev){
    ...
    if (ev.changedTouches && ev.changedTouches[0]) return { clientX: ev.changedTouches[0].clientX, clientY: ev.changedTouches[0].clientY };
    return { clientX: ev.clientX, clientY: ev.clientY };
  }

  function dragStartHandler(ev){
    // evita arrastar quando tocar em controles interativos
    const interactive = ev.target.closest && ev.target.closest('.wbq-btn, .wbq-sel, .wbq-inp, .wbq-tab');
    if (interactive) return;

    // permitir arrastar tanto collapsed quanto expanded a partir de qualquer área não-interativa
    S.dragging = true;
    const c = getClient(ev);
    S.dragStart = { x: c.clientX, y: c.clientY, pos: getCurrentPos() };
  ...
  function dragging(ev){
    ...
    // bloquear scroll do navegador durante arraste em touch
    if (ev.type === 'touchmove') try { ev.preventDefault(); } catch {}
    const c = getClient(ev);
    const dx = c.clientX - S.dragStart.x;
    const dy = c.clientY - S.dragStart.y;
    const vw = window.innerWidth, vh = window.innerHeight;
  ...
  // Clique único para expandir/colapsar (mas ignora cliques em controles)
  // Esse listener fica no root (elemento não é recriado) e garante abrir quando estiver colapsado.
  S.refs.root.addEventListener('click', (ev) => {
    if (ev.target.closest && ev.target.closest('.wbq-btn, .wbq-sel, .wbq-inp, .wbq-tab')) return;
    if (!S.expanded) toggleExpand();
  });
