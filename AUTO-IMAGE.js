...
;(async () => {
  ...
  const TEXTS = {
    pt: {
      ...
      selectPositionAlert:
        'Clique para pintar o primeiro pixel onde a arte começará.',
  ...
  const utils = {
    ...
    getPixelDelay: () => {
      switch (state.settings.speed) {
        case 'fast':
          return CONFIG.PIXEL_DELAY_FAST
        case 'safe':
          return CONFIG.PIXEL_DELAY_SAFE
        default:
          return CONFIG.PIXEL_DELAY_NORMAL
      }
    },
    shuffle: arr => {
  ...
  function buildPixelList() {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (state.settings.skipTransparent && a < CONFIG.TRANSPARENCY_THRESHOLD)
          continue
    ...
    window.updateUI &&
      window.updateUI('listRebuilt', 'success', { count: list.length })
  }
  ...
  function initUI() {
    ...
    function refreshTexts() {
      ...
      btnStop.querySelector('span').textContent = utils.t(
        state.running ? 'stopPainting' : 'stopPainting'
      )
    ...
    btnUpload.addEventListener('click', async () => {
      try {
        ...
        updateUI('imageLoaded', 'success', {
          count: state.imageData.totalPixels
        })
        updateStats()
      } catch {
    ...
    while (state.pixelIndex < state.pixelList.length) {
      if (state.stopFlag) {
        updateUI('paintingPaused', 'warning', {
          x: state.pixelList[state.pixelIndex]?.x ?? 0,
          y: state.pixelList[state.pixelIndex]?.y ?? 0
        })
        updateStats()
        return
      }
      ...
      const nearestColorId = mapToNearestColor(
        [px.r, px.g, px.b],
        state.availableColors
      )

      const painted = await api.paintPixelInRegion(
        regionX,
        regionY,
        baseX + px.x,
        baseY + px.y,
